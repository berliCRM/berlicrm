		...
		foreach($moduleFieldGroups as $blocklabel => $fieldgroups) {
			...
			*/
			...
			if($mailingAddressOK == 1) {
				if($module == 'Contacts') {
					$label = getTranslatedString("address", "crmtogo");
				} 
				else {
					$label = getTranslatedString("bill_address", "crmtogo");
				}
				$fields[] = array("name" => $fieldnames[10], "value" => $mailingAddress, "label" => $label, "uitype" => "crm_app_map", "typeofdata" => "O");
			}
			if($otherAddressOK == 1) {
				if($module == 'Contacts') {
					$label = getTranslatedString("otheraddress", "crmtogo");
				}
				else {
					$label = getTranslatedString("ship_address", "crmtogo");
				}
