...
class crmtogo_WS_Utils {
	...
	static function getVersion() {
		...
		$versionResult = $db->pquery("SELECT version FROM vtiger_tab WHERE name='crmtogo'", array());
	...
	**/
	...
	static function fixReferenceIdByModule($module, $fieldid) {
		...
		/** Leo 17-02-2017 Bug, no pueden seleccionar cuentas al crear una actividad */
	...
	* @auth Leo 
	* @date 26 - 01 -2017
	* @descript Gets the name of the account based on the id..
	*/
	// TODO: we need to generate a more generic function.
	static function getAccountName($wsId){

		if(self::detectModulenameFromRecordId($wsId) == 'Accounts'){
			$db = PearDatabase::getInstance();
			$id = explode("x", $wsId);
			$result = $db->pquery("SELECT accountname FROM vtiger_account WHERE accountid=?", array($id[1]));
			if($result && $db->num_rows($result)) {
	...
	*@auth Leo 
	*@date 27 - 01 - 2017
	*@description Checks if the user is assigned to the entity.
	*/
	static function isActivityOwner($assignedid,$userid){
		if(self::detectModulenameFromRecordId($assignedid) == 'Users'){
			$id = explode("x", $assignedid);
			if($id[1] == $userid){
				return true;
			}
		}else{
			...
			$db = PearDatabase::getInstance();
			$id = explode("x", $assignedid);
			$result = $db->pquery("SELECT userid from vtiger_users2group where groupid = ? and userid = ?", array($id[1],$userid));
			if($result && $db->num_rows($result)) {
				return true;
			}
		}
	...
	*@auth Leo 
	*@date 11-04-2017
	*@descrition Función para transformar la hora del usuario a utc 0.
	*/
