(function($) {
   $.jqmCalendar = function(element, options) {
		...
		var plugin = this,today = new Date();
		...
		function init() {
			...
			$tr = $("<tr/>").appendTo($thead);
			 ...
			 // The way of determining the labels for the days is a bit awkward, but works.
		...
		function listItemFormatter($listItem, timeString, summary, event) {
			...
			*@auth Leo
			*@date 30-01-2017
			*@descripton We remarked the activities realized to dark gray.
			*/
			if(event['isdone']){
				  $('<a style="background:#A4A4A4;"></a>').text( text ).attr( 'href', '?_operation=fetchRecord&record='+event[plugin.settings.id] ).appendTo($listItem);
			}
			else{
