...
var crmtogo_Index_Js = {
		...
		if (module != 'Calendar') {
			...
			$("#fliptoggle").on("slidestart", function(e){
				...
				if(show) {
					...
					$('#scopetoggle-button').css('display','block');
					...
					$('#scopetoggle-button').css('display','none');
							...
							if (calinfo=='records') {
								if (!$.isEmptyObject(calvalue)) {
									$.each( calvalue, function( calname, calcontent ){
										...
										*@auth Leo
										*@date 30-01-2017
										*@descript Añadimos el atributo isdone para descriminar actividades realizadas.
										*/
										isdone = calcontent['isdone'];
										caljson.push({'summary' :summary, 'begin': startdate,'end':enddate,'id':id, 'isdone':isdone});
										//Fin
									});
								}
							}
	...
	registerEventsForEditView : function(){
		...
		/* Leo 17-02-2017 Update end date with start date value */
		$('#date_start').change(function(){
			$('#due_date').val($('#date_start').val());
		});
	},
	...
	registerEventsForDetailView : function(event){
		...
		$("#savecomment").click(function(e){
			...
			function addComment() {
				...
				.fail(function() {
					alert( "Comment Save Error, please contact your CRM administrator to activate comments for your CRM module." );
					return false;
				});
			}
		});
