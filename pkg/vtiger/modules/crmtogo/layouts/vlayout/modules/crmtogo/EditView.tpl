...
<!DOCTYPE html>
<head>
	<title>{$_MODULE->label()} {vtranslate('LBL_EDIT', 'crmtogo')}</title> 
...
<div data-role="page" data-theme="b" data-mini="true" id="edit_page">
	...
	<div data-role="header" data-theme="{$COLOR_HEADER_FOOTER}" data-position="fixed">
		<a href="#"  class="ui-btn ui-corner-all ui-icon-check ui-btn-icon-notext" >{vtranslate('LBL_SAVE', 'crmtogo')}</a>			
		<h2>{vtranslate('LBL_EDIT', 'crmtogo')}</h2>
							...
							<div>   
								{if $_FIELD->uitype() eq '1' || $_FIELD->uitype() eq '2' || $_FIELD->uitype() eq '55' || $_FIELD->uitype() eq '255' || $_FIELD->uitype() eq '11'  || $_FIELD->uitype() eq '13'  || $_FIELD->uitype() eq '17' || $_FIELD->uitype() eq '72' || $_FIELD->uitype() eq '22'  || $_FIELD->uitype() eq '20'}
									...
									{else}
										...
										<input  type="text" name="{$_FIELD->name()}" id="{$_FIELD->name()}" value="{if $_FIELD->typeofdata()|strstr:'N'} {$_FIELD->valueLabel()|decimalFormat}{ELSE}{$_FIELD->valueLabel()|escape:'html'}{/if}" {if $_FIELD->ismandatory() eq 'M'}class="required"{/if} />
									{/if}
			   				    {/if}
								{if $_FIELD->uitype() eq '23' || $_FIELD->uitype() eq '5' || $_FIELD->uitype() eq '6' || $_FIELD->uitype() eq '252'}
										...
										{if $_FIELD->name() neq 'time_start' &&  $_FIELD->name() neq 'time_end'}
											{if $_FIELD->name() eq 'date_start'}
												...
												<label for="{$_FIELD->name()}">{vtranslate('Start Date & Time',$_MODULE->name())}{if $_FIELD->ismandatory() eq 'M'}*{/if}:</label>
											{else}
								...
								{if $_FIELD->uitype() eq '4'}
									<label for="{$_FIELD->name()}" >{$_FIELD->label()}{if $_FIELD->ismandatory() eq 'M'}*{/if}:</label>
									<input  type="text" class="ui-disabled" name="{$_FIELD->name()}" id="{$_FIELD->name()}" value="{$_FIELD->value()|escape:'html'}"  />
							    {/if}							
								{if $_FIELD->uitype() eq '15'}
										...
										<select  id="{$_FIELD->name()}" name="{$_FIELD->name()}"   data-mini="true" class="select" data-native-menu="false">
											{foreach item=arr from=$_FIELD->value()}
												{if $arr.label eq vtranslate('LBL_NOT_ACCESSIBLE', 'crmtogo')}
													<option value="{$arr.label}" {$arr.selected}>
														{$arr.label}
													</option>
												{else}
												<option value="{$arr.value}" {$arr.selected}>
													{vtranslate($arr.label,$_MODULE->name())}
							                    </option>
												{/if}
											{foreachelse}
												<option value=""></option>
												<option value="" style='color: #777777' disabled>{vtranslate('LBL_NONE', 'crmtogo')}</option>
											{/foreach}
										</select>
								{/if} 							
								...
								{if $_FIELD->uitype() eq '53'}
									...
									<div data-role="fieldcontain" >
									    <fieldset data-role="controlgroup" data-type="horizontal" data-mini="true" >
										    <label for="User">{vtranslate('LBL_USER', 'crmtogo')}</label>
											<input id="User"  type="radio"  name="assigntype" {$select_user} value="U" >
											<label for="Group">{vtranslate('LBL_GROUP', 'crmtogo')}</label>
                                ...
                                {/if}
 								{if $_FIELD->uitype() eq '56' && $_FIELD->name() eq 'sendnotification'}
								    <div>
										...
										<label for="{$_FIELD->label()}">{$_FIELD->label()}{if $_FIELD->ismandatory() eq 'M'}*{/if}:</label> 
										<input type="checkbox" name="{$_FIELD->name()}" id="{$_FIELD->name()}" class="custom" />
										{/if}
								    </div>
                                {/if}
								{if ($_FIELD->uitype() eq '10')||  ($_FIELD->uitype() eq '51')||  ($_FIELD->uitype() eq '59')||  ($_FIELD->uitype() eq '68') || ($_FIELD->uitype() eq '57') || ($_FIELD->uitype() eq '66')}
								...
								{if $_FIELD->uitype() eq '9'}
								    <div>
										...
										<input  type="text" name="{$_FIELD->name()}" id="{$_FIELD->name()}" value="{$_FIELD->valueLabel()|decimalFormat|escape:'html'}" {if $_FIELD->ismandatory() eq 'M'}class="required"{/if} />
								    </div>
							    {/if}		
							    ...
							    {/if}		
 								{if $_FIELD->uitype() eq '71' || $_FIELD->uitype() eq '7'}
								    <div>
										<label for="{$_FIELD->label()}">{$_FIELD->label()}{if $_FIELD->ismandatory() eq 'M'}*{/if}:</label>
										<input  type="text" name="{$_FIELD->name()}" id="{$_FIELD->name()}" value="{$_FIELD->valueLabel()|decimalFormat}" {if $_FIELD->ismandatory() eq 'M'}class="required"{/if} />
								    </div>
							    {/if}
                          </div>								
					{/foreach}
