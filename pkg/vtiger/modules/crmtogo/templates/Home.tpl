		...
		{foreach item=_MODULE from=$_MODULES}
			...
			<li id={$_MODULE->name()}>
				...
				{if file_exists($iconpath)}
					<a href="index.php?_operation=listModuleRecords&module={$_MODULE->name()}" class="ui-btn ui-btn-icon-left ui-icon-{$_MODULE->name()}" target="_blank">{$_MODULE->label()}</a>
				{else}
					<a href="index.php?_operation=listModuleRecords&module={$_MODULE->name()}" class="ui-btn ui-btn-icon-left ui-icon-Vtiger" target="_blank">{$_MODULE->label()}</a>
				{/if}
				<!-- Fin -->
			</li>
			{/if}
