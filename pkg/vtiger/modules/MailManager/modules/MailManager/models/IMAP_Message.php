...
class MailManager_IMAPMessage_Model extends Vtiger_MailRecord  {
	 ...
	 */
	public function __construct($message = false, $fetchbody=false) {
	 ...
	 * @access private
	 */
	function __parseHeader($message) {
		$this->_from = Array();
		$this->_to = Array();

		$this->_uniqueid = $this->mUid;
		$header = $message->getHeader();

		$this->_from = array(htmlentities($header->from[0]->full));
        $this->_fromname = $header->from[0]->personal;
		$this->_to = array(htmlentities($header->to->toString()));
		$cc = $header->cc;
		if ($cc) {
			// $cc = $cc->toString();
		}
		...
		$this->_date = $header->date->toDate()->timestamp;

		$this->_subject = $header->subject->toString();
		if(!$this->_subject) {
			$this->_subject = 'Untitled';
		}
	}
	
	function __parseBody($message) {
		$this->_plainmessage = '';
		$this->_htmlmessage = '';
		$this->_body = '';
		...
		if ($message->hasHTMLBody()) {
			$this->_body = $message->getHTMLBody();
			$this->_isbodyhtml = true;
		} else {
			$this->_body = $message->getTextBody();
		}
	...
	public function date($format = false) {
		$date = $this->_date;
		if ($date) {
			$date = Vtiger_Util_Helper::convertDateTimeIntoUsersDisplayFormat(date('Y-m-d H:i:s', $date));
		}
		return $date;
	}
	...
	public static function parseOverview($result) {
		...
		$instance->setFrom(htmlentities($result->from[0]->full));
		$instance->setDate($result->date->toDate()->timestamp);
		$instance->setRead($result->seen);
		$instance->setMsgNo($result->get('msgn'));
