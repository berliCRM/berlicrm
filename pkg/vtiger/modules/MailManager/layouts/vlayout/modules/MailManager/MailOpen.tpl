            ...
            <div class="contents" style="padding-right: 2.2%;">
                <div class="row-fluid">
                    <div class="span6">
						...
						<span id="_mailopen_fromcrmid" style="display:none">{$MAIL->_fromcrmid}</span><br>
                ...
                <span>
                    {foreach item=ATTACHVALUE key=ATTACHNAME from=$ATTACHMENTS name="attach"}
                        {if $INLINE_ATT[$ATTACHNAME] eq null}
                            <img border=0 src="{vimage_path('emailattachment.gif')}">
                            <a href="index.php?module={$MODULE}&view=Index&_operation=mail&_operationarg=attachment_dld&_muid={$MAIL->muid()}&_atname={$ATTACHNAME|urlencode}">{$ATTACHNAME}</a>
                            &nbsp;
                        {/if}
                    {/foreach}
