				...
				<td align="center" colspan="3">
					...
					<select class="small" onchange="MailManager.mail_associate_actions(this.form);" name="_mlinktotype"  id="_mlinktotype">
						...
						{foreach item=moduleName from=$LINK_TO_AVAILABLE_ACTIONS}
							<option id="default" value="{$moduleName}">{vtranslate("LBL_MAILMANAGER_ADD_$moduleName", 'MailManager')}</option>
						{/foreach}
							<option id="linktoother" value="add_to_contact">{vtranslate("LBL_COPY_MAIL_TO_OTHER", 'MailManager')}</option>
					</select>
					{/if}
				</td>
			</tr>
