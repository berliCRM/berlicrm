...
SCRIPTS RECORDID  MODULE  PARENTMODULE  DOCTYPE  DOCUMENTFOLDERS
...
<form class="form-horizontal" id="EditView" name="EditView" method="post" action="index.php" enctype="multipart/form-data">
    ...
    <div class="modal-body ">
        ...
        {vtranslate('LBL_FOLDER',$MODULE)}
        <span class="redColor">
            *
        </span>
        <br>
        <select name="folder_selection" id="folder_selection" style="width:200px;">
            {assign "SAVED_FOLDERID" $DOCSETTINGS['folderid']}
            {foreach item=FOLDER key=FOLDERID from=$DOCUMENTFOLDERS}
                <option value="{$FOLDERID}" {if $FOLDERID eq $SAVED_FOLDERID}selected{/if}>
                    {if $FOLDER=="Default"}
                        Standardverzeichnis
                    {else}
                        {$FOLDER}
	...
	<div class="dropcontainer" style="position: relative;">
		...
		<div class="upload-area" style="display: flex; margin-left:5px; margin-right:5px;margin-top: 5px; margin-bottom: 5px; border: 2px dashed #ccc; height: 120px; text-align: center; justify-content: center; align-items: center; " id="uploadfile">
			<input type="file" name="file" id="file" multiple style="opacity: 0.0; position: absolute; top:0; left: 0; bottom: 0; right:0; width: 100%; height:100%;" />
			<p>
                {vtranslate('LBL_FILE_DRAGANDDROP')} 
            </p>
		</div>
	</div>
</form>
