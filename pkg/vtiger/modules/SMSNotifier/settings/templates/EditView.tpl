...
<div class="modal">
	...
	<form class="form-horizontal" id="smsConfig"  name="smsConfig"  method="POST">
			...
			<div class="control-group">
				...
				<div class="controls">
					...
					{if $FIELD_TYPE == 'picklist'}
						<select class="chzn-select providerType" name="{$FIELD_NAME}" id="{$FIELD_NAME}" data-validation-engine="validate[ required,funcCall[Vtiger_Base_Validator_Js.invokeValidation]]">
							<option></option>
					...
					{else}
						<input type="text" name="{$FIELD_NAME}" id="{$FIELD_NAME}" class="span3" {if $FIELD_NAME == 'username'} data-validation-engine="validate[required]" {/if} value="{$FIELD_VALUE}" />
					{/if}
				</div>
			</div>
			{/foreach}
