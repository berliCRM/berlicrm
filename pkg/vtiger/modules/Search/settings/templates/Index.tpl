			...
			<table class="table table-bordered table-condensed listViewEntriesTable" id="modulesEntity">
				<thead>
					<tr class="blockHeader">
						...
						<th><strong>{vtranslate('LBL_SEARCH_ALL',$QUALIFIED_MODULE)}</strong></th>
						<th><strong>{vtranslate('LBL_SAVE_SETTINGS',$QUALIFIED_MODULE)}</strong></th>
				...
				<tbody>
					{foreach from=$ModulesEntity item=item key=key}
						...
						<tr data-tabid="{$key}">
							...
							<td>{if $item['searchall'] eq 1}
								<input type="checkbox" name="globalsearchall" class="globalsearchall" id="globalsearchall" checked="checked" value="{$item['searchall']}">&nbsp;{vtranslate('LBL_SEARCH_ALL',$QUALIFIED_MODULE)}
								{else}
								<input type="checkbox" name="globalsearchall"  class="globalsearchall" id="globalsearchall" value="{$item['searchall']}">&nbsp;{vtranslate('LBL_SEARCH_ALL',$QUALIFIED_MODULE)}
								{/if}
							</td>
							<td>
