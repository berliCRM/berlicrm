...
Vtiger_Detail_Js("berliCleverReach_Detail_Js",{},{
	
	loadRelatedList : function(pageNumber){
		var relatedListInstance = new berliCleverReach_RelatedList_Js(this.getRecordId(), app.getModuleName(), this.getSelectedTab(), this.getRelatedModuleName());
	...
	registerEventForRelatedListPagination : function(){
		...
		detailContentsHolder.on('click','#relatedListNextPageButton',function(e){
			...
			var relatedController = new berliCleverReach_RelatedList_Js(thisInstance.getRecordId(), app.getModuleName(), selectedTabElement, relatedModuleName);
	...
	registerEventForRelatedList : function(){
		...
		detailContentsHolder.on('click', 'button.massDeleteRelations', function(e){
			...
			var url = 'berliCleverReach_Detail_Js.triggerMassDeleteRelations("index.php?module=$relatedModuleName&action=MassDelete")'; // what?
				...
				Vtiger_Helper_Js.showConfirmationBox({'message' : message}).then(
					function(e) {
						...
						if(selectedIds != 'all'){
							...
							$.when(listInstance.getAllRecordIds()).done(function(selectedIds){
								selectedIds = jQuery.parseJSON(selectedIds);
								for (index = 0; index < selectedIds.length; ++index) {
								   relatedRecordid = selectedIds[index];
									relatedController.deleteRelation([relatedRecordid]).then(function(response){
		...
		jQuery('.related', detailContainer).on('click', 'li', function(e, urlAttributes){
			...
			thisInstance.loadContents(url,urlAttributes).then(
				function(data){
					...
					var emailEnabledModule = jQuery(data).find('[name="emailEnabledModules"]').val();
					if(emailEnabledModule){
						var listInstance = new berliCleverReach_List_Js();
						listInstance.registerEvents();
	...
	registerRelatedListEvents : function(){
		...
		var relatedController = new berliCleverReach_RelatedList_Js(this.getRecordId(), app.getModuleName(), selectedTabElement, relatedModuleName);
	...
	registerEvents : function(){
		this.registerRelatedListEvents();
		this._super();
		//Calling registerevents of clever reach list to handle checkboxs click of related records
		var listInstance = Vtiger_List_Js.getInstance();
		listInstance.registerEvents();
	}
})