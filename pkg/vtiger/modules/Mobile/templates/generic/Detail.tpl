...
<div data-role="page" data-theme="b">
	...
	<div data-role="header" class="ui-bar" data-mini='true' data-theme="b">
		...
		<a href="?_operation=edit&module={$_MODULE->name()}&record={$_RECORD->id()}" data-mini='true' data-role="button" data-prefetch>{vtranslate('LBL_EDIT', 'Mobile')}</a>
		{/if}
		<a data-role="button" data-inline="true" href="index.php?_operation=listModuleRecords&module={$_MODULE->name()}" data-mini='true' rel=external>{vtranslate('LBL_LISTVIEW', 'Mobile')}</a>
		</div>
	</div>
	{else}
	<div data-role="header" class="ui-bar" data-theme="b">
		<div class="ui-grid-b" data-role="controlgroup" data-type="horizontal">
				<a href="?_operation=edit&module={$_MODULE->name()}&record={$_RECORD->id()}" data-role="button"  data-prefetch>{vtranslate('LBL_EDIT', 'Mobile')}</a>
				<a href="?_operation=getrelatedlists&module={$_MODULE->name()}&record={$_RECORD->id()}" data-role="button"  data-prefetch>{vtranslate('LBL_RELATED_LISTS', 'Mobile')}</a>
				<a href="index.php?_operation=listModuleRecords&module={$_MODULE->name()}"  data-role="button"   rel=external>{vtranslate('LBL_LISTVIEW', 'Mobile')}</a>
	...
	{if isset($_COMMENTS) && ($COMMENTDISPLAY eq true)}
		<div data-role="collapsible" data-collapsed="true" data-mini="true">
			<h3>{vtranslate('ModComments', 'ModComments')}</h3>
				...
				{include file='modules/crmtogo/generic/Comments.tpl'}
				</div>
			<div data-role="main" class="ui-content">
			  <div class="ui-field-contain">
				<textarea name="comment_text" id="comment_text"></textarea>
			  </div>
			  <input type="submit" data-inline="true" value="{vtranslate('LBL_SAVE', 'Mobile')}" onClick="addComment('{$_RECORD->id()}');">
			</div>
		</div>
	{/if}	
									...
									{else}
										{if $_FIELD->uitype() eq '56'}
											{if $_FIELD->valueLabel() eq '1'}
												{vtranslate('LBL_YES', 'Mobile')}
											{else}
												{vtranslate('LBL_NO', 'Mobile')}
											{/if}
										{else}
											{if ($_FIELD->name() neq 'eventstatus' && $_FIELD->name() neq 'taskstatus') || $_FIELD->valueLabel() neq ''}
												{$_FIELD->valueLabel()}
	...
	<div class="ui-grid-b">
		<a href="?_operation=deleteConfirmation&module={$_MODULE->name()}&record={$_RECORD->id()}&&lang={$LANGUAGE}" data-mini='true' data-role='button' data-inline='true' data-rel="dialog" data-transition="turn" data-prefetch>{vtranslate('LBL_DELETE', 'Mobile')}</a>
	</div> 
</div>
</body>