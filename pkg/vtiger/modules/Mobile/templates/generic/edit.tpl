							...
							<div>   
								...
								{if $_FIELD->uitype() eq '23' || $_FIELD->uitype() eq '5' || $_FIELD->uitype() eq '6' || $_FIELD->uitype() eq '252'}
										...
										{if $_FIELD->name() neq 'time_start' &&  $_FIELD->name() neq 'time_end'}
											...
											{/if}
											<input data-mini="true" type="text" name="{$_FIELD->name()}" id="{$_FIELD->name()}" value="{$_FIELD->value()|date_format:$dateFormat}" {if $_FIELD->typeofdata() eq 'M'}class="required"{/if} />
												{literal}
										...
										{if $_FIELD->uitype() eq '252' && $_FIELD->name() eq 'time_start'}
											...
											<input type="text" name="time_start" id="time_start" value="{$time_value}" class="required" />
											{literal}
												<script type="text/javascript">
													$(document).ready(function () {
														$('#time_start').show();
										...
										{if $_FIELD->uitype() eq '252' && $_FIELD->name() eq 'time_end'}
											<input type="hidden" name="time_end" id="time_end" value="{$time_value}" {if $_FIELD->typeofdata() eq 'M'}class="required"{/if} />
										{/if}
								{/if}			
