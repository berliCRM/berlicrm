...
class CRMEntity
{
     ...
     *      return void
     */
    public function uploadAndSaveFile($id, $module, $file_details, $attachmentType = 'Attachment', $internalFile = false)
    {
        global $log;
    ...
    public function insertIntoCrmEntity($module, $fileid = '')
    {
        ...
        if ($this->mode == 'edit') {
            ...
            if (is_array($description_val)) {
                $description_val = json_encode($description_val);
            }
            ...
            if ($is_admin == true || $profileGlobalPermission[1] == 0 || $profileGlobalPermission[2] == 0) {
                $params = [$ownerid, $current_user->id, $description_val, $adb->formatDate($date_var, true), $this->id];
            } else {
                ...
                if (is_array($columname) && in_array("description", $columname)) {
                    ...
                    $params = [$ownerid, $current_user->id, $adb->formatDate($date_var, true), $this->id];
                }
            }
            ...
            $params = [$current_id, $current_user->id, $ownerid, $module, $description_val, $current_user->id, $created_date_var, $modified_date_var];
            $adb->pquery($sql, $params);
    ...
    public function findRecurringreferencesIDs($idRef)
    {
        ...
        $resultRef1 = $adb->pquery($sqlRef1, [ $idRef ]);
        ...
        if ($rowsRef1 > 0) {
            ...
            if ($parentactivityidMe != '') { // (here it muss have a value. )
                ...
                $resultRef2 = $adb->pquery($sqlRef2, [  $parentactivityidMe  ]);
    ...
    public function insertIntoEntityTable($table_name, $module, $fileid = '')
    {
        ...
        if ($insertion_mode == 'edit' && $this->column_fields['recurringtype'] != '' && $this->column_fields['recurringtype'] != '--None--') {
            ...
            if (count($idsReferencesArray) > 0) {
                ...
                $resultRef2 = $adb->pquery($sqlRef2, [  $idsReferencesArray[0]  ]);
                $rowsRef2 = $adb->num_rows($resultRef2);
                if ($rowsRef2 > 0) {
                ...
                for ($a = 0; $a < count($idsReferencesArray) ; $a++) {
                    ...
                    $sqlRef1 = 'SELECT * FROM `berlicrm_recurringreferences` WHERE activityid = ?';
                    $resultRef1 = $adb->pquery($sqlRef1, [ $idsReferencesArray[$a] ]);
                    $rowsRef1 = $adb->num_rows($resultRef1);
                ...
                for ($a = 0; $a < count($idsReferencesArray) ; $a++) {
                    ...
                    $adb->pquery($deleteFromBerlicrm_recurringreferences, [  $idsReferencesArray[$a]  ]);
                    ...
                    if ($this->id == $idsReferencesArray[$a]) {
                        ...
                        $adb->pquery($delete_from_vtiger_activity, [  $idsReferencesArray[$a]  ]);
                        ...
                        $adb->pquery($delete_from_vtiger_activity_reminder, [$idsReferencesArray[$a]]);

                        $delete_from_vtiger_cntactivityrel = 'DELETE FROM vtiger_cntactivityrel WHERE activityid = ?';
                        $adb->pquery($delete_from_vtiger_cntactivityrel, [$idsReferencesArray[$a]]);
                    }
                }
        ...
        if ($insertion_mode == 'edit') {
            if ($module == 'Potentials') {
                ...
                if ($sales_stage != $_REQUEST['sales_stage'] && $_REQUEST['sales_stage'] != '') {
                    ...
                    $params = ['', $this->id, $this->column_fields['amount'], decode_html($sales_stage), $this->column_fields['probability'], 0, $adb->formatDate($closingdate, true), $adb->formatDate($date_var, true)];
                    $adb->pquery($sql, $params);
                }
    ...
    public function save($module_name, $fileid = '')
    {
        ...
        if ($em) {
            ...
            $res = $adb->pquery($q, [$entityData->getModuleName()]);
            ...
            if ($res && $adb->num_rows($res) > 0 && isset($_REQUEST["operation"]) && ($_REQUEST["operation"] == "create" || $_REQUEST["operation"] == "update" || $_REQUEST["operation"] == "revise")) {
                $_SESSION["delayedtrigger"] = ["em" => $em, "data" => $entityData];
            } else {
                //Event triggering code
    ...
    public function unlinkRelationship($id, $return_module, $return_id)
    {
        ...
        $fieldRes = $this->db->pquery('SELECT tabid, tablename, columnname FROM vtiger_field WHERE fieldid IN (
			SELECT fieldid FROM vtiger_fieldmodulerel WHERE module=? AND relmodule=?)', [$currentModule, $return_module]);
        $numOfFields = $this->db->num_rows($fieldRes);
        for ($i = 0; $i < $numOfFields; $i++) {
            $tabId = $this->db->query_result($fieldRes, $i, 'tabid');
            $tableName = $this->db->query_result($fieldRes, $i, 'tablename');
    ...
    public function setModuleSeqNumber($mode, $module, $req_str = '', $req_no = '')
    {
        ...
        if ($mode == "configure" && $req_no != '') {
            ...
            $check = $adb->pquery("SELECT cur_id,prefix FROM vtiger_modentity_num WHERE semodule=? AND active = 1 FOR UPDATE", [$module]);
    ...
    public function get_related_list($id, $cur_tab_id, $rel_tab_id, $actions = false)
    {
        ...
        if (empty($other->related_tables)) {
            $other->related_tables = [$cf_tablename => [$other->customFieldTable[1]]];
        }
        if (!empty($other->related_tables)) {
    ...
    public function getRelationQuery($module, $secmodule, $table_name, $column_name, $queryPlanner)
    {
        ...
        if ($pritablename == 'vtiger_crmentityrel') {
            ...
            $relTmpQuery = $queryPlanner->registerTempTable($relTmpQuery, ['crmid', 'relcrmid']);
