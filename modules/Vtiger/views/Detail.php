...
class Vtiger_Detail_View extends Vtiger_Index_View {
    ...
    protected function assignTicketStatusForCommentForm($viewer, $moduleName, $parentRecordId) {
        ...
        if(!Users_Privileges_Model::isPermitted('HelpDesk', 'Save', $parentRecordId)) {
            return;
        }

        $parentRecord = Vtiger_Record_Model::getInstanceById($parentRecordId, $moduleName);
        $ticketStatusField = $parentRecord->getModule()->getField('ticketstatus');
        if(empty($ticketStatusField) || !$ticketStatusField->isEditable()) {
            return;
        }

        $currentTicketStatus = $parentRecord->get('ticketstatus');
        $ticketStatusPicklistValues = $ticketStatusField->getPicklistValues();
        if(empty($ticketStatusPicklistValues)) {
            $ticketStatusPicklistValues = array();
        }
        if($currentTicketStatus !== '' && !array_key_exists($currentTicketStatus, $ticketStatusPicklistValues)) {
            $ticketStatusPicklistValues = array($currentTicketStatus => $ticketStatusField->getDisplayValue($currentTicketStatus)) + $ticketStatusPicklistValues;
        }

        $viewer->assign('TICKET_STATUS_FIELD_MODEL', $ticketStatusField);
        $viewer->assign('TICKET_STATUS_PICKLIST_VALUES', $ticketStatusPicklistValues);
        $viewer->assign('CURRENT_TICKET_STATUS', $currentTicketStatus);
    }
    /**
