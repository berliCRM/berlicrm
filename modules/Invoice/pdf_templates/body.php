...
if ($invoice_status == 'Credit Invoice' and ($purpose != 'printsn' and $purpose != 'savesn')) {
    $pdf->Cell($pdf->GetStringWidth($pdf_strings['CREDIT']), $pdf->getFontSize(), $pdf_strings['CREDIT'], 0, 1);
} elseif ($purpose == 'printsn' or $purpose == 'savesn') {
    $pdf->Cell($pdf->GetStringWidth($pdf_strings['SALESNOTE']), $pdf->getFontSize(), $pdf_strings['SALESNOTE'], 0, 1);
} else {
...
if ($focus->column_fields["hdnTaxType"] == "individual") {
    $column_body_content = $pdfcolumnsettings[1];
} else {
    $column_body_content = $pdfcolumnsettings[0];
}
...
foreach ($column_body_content as $key => $value) {
    if ($value['selected'] == 'checked="checked"' and $value['enabled'] == '1') {
        ...
        if ($focus->column_fields["hdnTaxType"] == 'individual') {
            $defined_columnsizes = array('Position' => '10','OrderCode' => '20','Description' => '21','Qty' => '15','Unit' => '20','UnitPrice' => '22','Discount' => '15','Tax' => '32','LineTotal' => '25');
        } else {
            $defined_columnsizes = array('Position' => '10','OrderCode' => '20','Description' => '53','Qty' => '15','Unit' => '20','UnitPrice' => '22','Discount' => '15', 'LineTotal' => '25');
        }
...
for ($i = 0;$i < $num_products;$i++) {
    ...
    for ($k = 0;$k < $total_length;$k++) {
        foreach ($cols as $lib => $pos) {
            ...
            if ($lib == $pdf_strings['Description']) {
                $total_length = $row_count_orig_formated_text;
            }
...
if ($summaryradio == 'true') {
    ...
    if ($newpage == true) {
        ...
        $pdf->SetXY(PDF_MARGIN_LEFT, $line_y_location + 5);
    }
    $line_y_location = $pdf->GetY();
    if ($focus->column_fields["hdnTaxType"] == "individual") {
    ...
    if ($invoice_received > 0) {
        // file_put_contents('logs/ep4812.log', sprintf('in body: %s', $invoice_received) . PHP_EOL, FILE_APPEND);
        $line_y_location = $line_y_location + 2;
        $pdf->SetY($line_y_location);
        //line & text $invoice_received
        $data = $pdf_strings['VAR_RECEIVED']." ($currency_symbol):";
        $pdf->SetXY(105, $line_y_location);
        $pdf->SetFont($default_font, "B", $font_size_body);
        $pdf->Cell(110, $line_distance, decode_html($data), 0, 0, 'L', 0, 0);
        //value $invoice_received
        $pdf->SetXY(144, $line_y_location);
        $pdf->Cell(54, $line_distance, decode_html(number_format($invoice_received, $decimal_precision, $decimals_separator, $thousands_separator)), 0, 1, 'R', 0, 1);
        $line_y_location = $pdf->GetY();

        // line for rest/due amount
        $line_y_location = $line_y_location + 2;
        $pdf->SetY($line_y_location);
        $data = $pdf_strings['VAR_OPEN']." ($currency_symbol):";
        $pdf->SetXY(105, $line_y_location);
        $pdf->SetFont($default_font, "B", $font_size_body);
        $pdf->Cell(110, $line_distance, decode_html($data), 0, 0, 'L', 0, 0);
        //value due amount
        $due_amount = $price_total - $invoice_received;
        $pdf->SetXY(144, $line_y_location);
        $pdf->Cell(54, $line_distance, decode_html(number_format($due_amount, $decimal_precision, $decimals_separator, $thousands_separator)), 0, 1, 'R', 0, 1);
        $line_y_location = $pdf->GetY();
    }
