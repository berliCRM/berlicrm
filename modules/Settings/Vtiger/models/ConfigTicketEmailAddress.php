 ...
 *  - vtiger_settings_config_param (param_key = 'enabled', 'sender_email', 'sender_name', 'reply_to_email', 'reply_to_name', 'attachment_folder_id')
 *
...
class Settings_Vtiger_ConfigTicketEmailAddress extends Vtiger_Base_Model {
    ...
    const PARAM_ATTACHMENT_FOLDER_ID = 'attachment_folder_id';
    ...
    /**
     * Return fallback document folder id.
     *
     * @return int
     */
    protected function getDefaultAttachmentFolderId() {
        return 1;
    }

    /**
     * Return a valid document folder id for uploaded ticket mail attachments.
     *
     * @param mixed $folderId
     * @return int
     */
    protected function normalizeAttachmentFolderId($folderId) {
        $folderId = (int)$folderId;
        if ($folderId <= 0) {
            return $this->getDefaultAttachmentFolderId();
        }

        $db = PearDatabase::getInstance();
        $result = $db->pquery(
            "SELECT folderid FROM vtiger_attachmentsfolder WHERE folderid = ? LIMIT 1",
            [$folderId]
        );
    ...
    /**
     * Return all document folders for settings picklists.
     *
     * @return array
     */
    public function getAttachmentFolders() {
        $db = PearDatabase::getInstance();
        $result = $db->pquery(
            "SELECT folderid, foldername FROM vtiger_attachmentsfolder ORDER BY sequence, foldername",
            []
        );
        ...
        if ($result) {
            ...
            for ($i = 0; $i < $rowCount; $i++) {
                $folderId = (int)$db->query_result($result, $i, 'folderid');
                $folders[$folderId] = (string)$db->query_result($result, $i, 'foldername');
            }
        }
    ...
    public function load() {
        ...
        $this->ensureParamExists($db, $configId, self::PARAM_ATTACHMENT_FOLDER_ID, 'reference', 5);
        ...
        $paramsResult = $db->pquery(
            ...
            [
                ...
                self::PARAM_ATTACHMENT_FOLDER_ID
            ]
        );
        ...
        $data = [
            ...
            self::PARAM_REPLYTO_EMAIL => '',
            self::PARAM_REPLYTO_NAME => '',
            self::PARAM_ATTACHMENT_FOLDER_ID => $this->getDefaultAttachmentFolderId(),
        ];
        ...
        $this->set('attachment_folder_id', $this->normalizeAttachmentFolderId($data[self::PARAM_ATTACHMENT_FOLDER_ID]));
    ...
    public function getData() {
        ...
        $attachmentFolderId = (int)$this->get('attachment_folder_id');
        $attachmentFolders = $this->getAttachmentFolders();
        ...
        return [
            ...
            'attachment_folder_name' => isset($attachmentFolders[$attachmentFolderId]) ? $attachmentFolders[$attachmentFolderId] : '',
    ...
    /**
     * Return the configured document folder id for ticket comment mail attachments.
     *
     * @return int
     */
    public function getAttachmentFolderId() {
        return $this->normalizeAttachmentFolderId($this->get('attachment_folder_id'));
    }
     ...
     * @param int $attachmentFolderId
     * @return self
     */
    public function save($enabled, $senderEmail, $senderName = '', $replyToEmail = '', $replyToName = '', $attachmentFolderId = 1) {
        $db = PearDatabase::getInstance();
        ...
        $attachmentFolderId = $this->normalizeAttachmentFolderId($attachmentFolderId);
        ...
        $this->upsertParam($db, $configId, self::PARAM_ATTACHMENT_FOLDER_ID, (string)$attachmentFolderId, 'reference', 5, $now);
        ...
        $this->set('attachment_folder_id', $attachmentFolderId);
