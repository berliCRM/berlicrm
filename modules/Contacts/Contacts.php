...
class Contacts extends CRMEntity
{
    ...
    */
    public function insertIntoAttachment($id, $module)
    {
		...
		WHERE contactid = ?";
        $adb->pquery($sqlImageInfoUpdate, [$imageName,$id]);
    ...
    public function transferRelatedRecords($module, $transferEntityIds, $entityId)
    {
        ...
        foreach ($transferEntityIds as $transferId) {
			...
			WHERE related_to = ?";
            $adb->pquery($updateSql, [$entityId, $transferId]);
        }
    ...
    public function save_related_module($module, $crmid, $with_module, $with_crmids)
    {
        ...
        foreach ($with_crmids as $with_crmid) {
            if ($with_module == 'Products') {
                $adb->pquery("insert into vtiger_seproductsrel values (?,?,?)", [$crmid, $with_crmid, 'Contacts']);
                ...
                $checkpresence = $adb->pquery("SELECT contactid FROM vtiger_cntactivityrel WHERE contactid = ? AND activityid = ?", [$crmid, $with_crmid]);
                // Does the relation already exists? Do not add it again
                if ($checkpresence && $adb->num_rows($checkpresence)) {
                    continue;
                }
                $adb->pquery("insert into vtiger_cntactivityrel values (?,?)", [$crmid,$with_crmid]);
            } else {
