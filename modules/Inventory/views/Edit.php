        ...
        if(!empty($record)  && $request->get('isDuplicate') == true) {
            ...
            if ($request->get('salesorder_id')) {
                ...
                for($i = 0;$i < count($relatedProducts);$i++) {
                    $relatedProducts[($i + 1)]['comment'.($i + 1)] = Invoice::replaceDatePlaceholder($relatedProducts[($i + 1)]['comment'.($i + 1)]);
                    $relatedProducts[($i + 1)]['productDescription'.($i + 1)] = Invoice::replaceDatePlaceholder($relatedProducts[($i + 1)]['productDescription'.($i + 1)]);
                }
            }
