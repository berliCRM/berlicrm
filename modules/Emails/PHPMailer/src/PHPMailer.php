...
class PHPMailer
{
    ...
    public function preSend()
    {
        ...
        try {
            ...
            if (count($this->to) + count($this->cc) + count($this->bcc) < 1) {
                $this->throwException($this->lang('provide_address'), self::STOP_CRITICAL);
            }
            ...
            if (!$this->AllowEmpty && empty($this->Body)) {
                $this->throwException($this->lang('empty_message'), self::STOP_CRITICAL);
            }
    ...
    protected function sendmailSend($header, $body)
    {
        ...
        if ($this->SingleTo) {
            foreach ($this->SingleToArray as $toAddr) {
                ...
                if (!$mail) {
                    $this->throwException($this->lang('execute') . $this->Sendmail, self::STOP_CRITICAL);
                }
    ...
    protected function mailSend($header, $body)
    {
        ...
        if (!$result) {
            $this->throwException($this->lang('instantiate'), self::STOP_CRITICAL);
        }
    ...
    protected function smtpSend($header, $body)
    {
        ...
        if (!$this->smtpConnect($this->SMTPOptions)) {
            $this->throwException($this->lang('smtp_connect_failed'), self::STOP_CRITICAL);
        }
        ...
        if (!$this->smtp->mail($smtp_from)) {
            ...
            $this->throwException($this->ErrorInfo, self::STOP_CRITICAL);
        }
        ...
        if ((count($this->all_recipients) > count($bad_rcpt)) && !$this->smtp->data($header . $body)) {
            $this->throwException($this->lang('data_not_accepted'), self::STOP_CRITICAL);
        }
        ...
        if (count($bad_rcpt) > 0) {
            ...
            $this->throwException($this->lang('recipients_failed') . $errstr, self::STOP_CONTINUE);
        }
    ...
    public function smtpConnect($options = null)
    {
        ...
        foreach ($hosts as $hostentry) {
            ...
            if (static::ENCRYPTION_STARTTLS === $secure || static::ENCRYPTION_SMTPS === $secure) {
                ...
                if (!$sslext) {
                    $this->throwException($this->lang('extension_missing') . 'openssl', self::STOP_CRITICAL);
                }
            }
            $host = $hostinfo[2];
            $port = $this->Port;
            ...
            if ($this->smtp->connect($prefix . $host, $port, $this->Timeout, $options)) {
                try {
                    ...
                    if ($this->SMTPAuth && !$this->smtp->authenticate(
                        ...
                        $this->throwException($this->lang('authenticate'));
                    }
        ...
        if ($this->isError()) {
            ...
            try {
                if (!defined('PKCS7_TEXT')) {
                    $this->throwException($this->lang('extension_missing') . 'openssl');
                }
                ...
                if ($sign) {
                    ...
                    @unlink($signed);
                    $this->throwException($this->lang('signing') . openssl_error_string());
                }
            } catch (Exception $exc) {
                $body = '';
                if ($this->exceptions) {
    ...
    public function addAttachment(
        ...
        try {
            if (!static::isPermittedPath($path) || !@is_file($path)) {
                $this->throwException($this->lang('file_access') . $path, self::STOP_CONTINUE);
            }
            ...
            if (!$this->validateEncoding($encoding)) {
                $this->throwException($this->lang('encoding') . $encoding);
            }
    ...
    protected function encodeFile($path, $encoding = self::ENCODING_BASE64)
    {
        try {
            if (!static::isPermittedPath($path) || !file_exists($path)) {
                $this->throwException($this->lang('file_open') . $path, self::STOP_CONTINUE);
            }
