			...
			<tr class="listViewHeaders">
				...
				{foreach key=LISTVIEW_HEADER_KEY item=LISTVIEW_HEADER from=$LISTVIEW_HEADERS}
					<th nowrap {if $LISTVIEW_HEADER@last} colspan="2" {/if}>
						<a href="javascript:void(0);" {if $LISTVIEW_HEADER_KEY != 'last_used'}class="listViewHeaderValues"{/if} data-nextsortorderval="{if $COLUMN_NAME eq $LISTVIEW_HEADER_KEY}{$NEXT_SORT_ORDER}{else}ASC{/if}" data-columnname="{$LISTVIEW_HEADER_KEY}">{vtranslate($LISTVIEW_HEADERS[$LISTVIEW_HEADER_KEY],$MODULE)}
