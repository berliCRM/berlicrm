			...
			<form id="ConfigTicketMailSenderForm"
				...
				<table class="table table-bordered table-condensed themeTableColor">
					...
					<tr>
						...
						<td style="border-left:none;" class="row-fluid {$WIDTHTYPE}">
							<span class="span3">
								<select class="select2 row-fluid" name="attachment_folder_id">
									{foreach key=FOLDER_ID item=FOLDER_NAME from=$DOCUMENT_FOLDERS}
										<option value="{$FOLDER_ID}" {if $FOLDER_ID eq $FIELD_DATA.attachment_folder_id}selected="selected"{/if}>
											{$FOLDER_NAME}
										</option>
									{/foreach}
								</select>
