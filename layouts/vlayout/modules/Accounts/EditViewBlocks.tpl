			...
			{/if}
                <thead>
                    <tr>
                        ...
                        {if $BLOCK_LABEL == "LBL_ADDRESS_INFORMATION"}
                        <div style="float:right;font-weight:normal;">{vtranslate("LBL_COPY_ADDRESS_TO_RELATED_CONTACTS", $MODULE)} <input type="checkbox" name="copytorelatedcontacts" style='vertical-align:text-top'></div>
                        {/if}
                    ...
                    <tr>
                        ...
                        {foreach key=FIELD_NAME item=FIELD_MODEL from=$BLOCK_FIELDS name=blockfields}
							...
							{if $FIELD_MODEL->get('helpinfo') && {$FIELD_MODEL->get('helpinfo')|trim} neq '' }
								<a style="margin-left: 2px;" href="#" class=" pull-right" rel="popover" title="{vtranslate('LBL_HELP', $MODULE)}" data-placement="top" data-trigger="hover" data-content="{vtranslate($FIELD_MODEL->get('helpinfo'), $MODULE)}"><i class="icon-info-sign"></i></a>
                            {/if}
